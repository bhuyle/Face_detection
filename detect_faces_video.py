...
def api_face(frame, x, y, x1, y1):
	...
	if not t.isAlive():
		x = Queue()
		t = threading.Thread(target=get_info, args=(image_read,))
		# t = threading.Thread(target=test_info,args=(x,))
		t.start()
		# print("New: ",x.get())
		t.join()
	...
	# 	cv2.putText(frame, str(data[0])+' '+str(data[1]), (x + 3, y1 + 12), font, 0.5, (255, 255, 255), 2)
	return frame
